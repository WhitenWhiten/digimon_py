...
def clean_battle_field(battle_id: int) -> None:
    (id1, id2) = battle_fields.pop(battle_id)
    in_battle.remove(id1)
    in_battle.remove(id2)
    
def get_fighters(battle_id):
    battle = battle_fields[battle_id]
...
def generate_process(battle_id) -> str:
    id1, id2 = get_fighters(battle_id)
    ...
    waiter = slower
    while True:
        dmg = eval('dmg' + str(acter))
        damage = random.randint(dmg[0], dmg[1])
        skill = random.choice(eval('skills' + str(acter)))
        acter_name = eval('digimon_name' + str(acter))
        ...
        acter = waiter 
        waiter = temp
    #退出while时，acter是谁，谁就是winer(因为退出条件是waiter血量<=0)
    winner = eval('mon' + str(acter))
    winner_name = eval('digimon_name' + str(acter))
    ...
    winner.victory()
    loser.defeated()
    winner.update_hp(eval('hp' + str(acter))[0])
    loser.update_hp(0)
    winner.evolve()
    loser.evolve()
